...
echo ""
echo "=========================================="
echo "  Comparative Results (seconds)"
echo "=========================================="
printf "  %-20s %15s %15s\n" "Metric" "Original" "Optimized"
echo "  --------------------------------------------------"
printf "  %-20s %15s %15s\n" "nargo_build" "${original_build_time:-error}" "${optimized_build_time:-error}"
printf "  %-20s %15s %15s\n" "nargo_execute" "${original_execute_time:-error}" "${optimized_execute_time:-error}"
printf "  %-20s %15s %15s\n" "bb_write_vk" "${original_write_vk_time:-error}" "${optimized_write_vk_time:-error}"
printf "  %-20s %15s %15s\n" "bb_prove" "${original_prove_time:-error}" "${optimized_prove_time:-error}"
printf "  %-20s %15s %15s\n" "bb_verify" "${original_verify_time:-error}" "${optimized_verify_time:-error}"
echo "=========================================="
echo ""
